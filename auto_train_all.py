...
def main():
    # =========================================================
    # PVRP + SNAA 消融实验任务列表（v3 — 修复版）
    # ---------------------------------------------------------
    # 相比 v2 的关键改动：
    #   1. SNAA Loss 完整实现：加入中心偏差归一化 + 归属边界惩罚(repulsion)
    #   2. P2Proxy 内部分支 n=1→2（更强的细节处理能力）
    #   3. NDA context kernel 7×7→5×5（适配小目标）
    #   4. lr0: 0.0005→0.001（标准 AdamW 学习率）
    #   5. box: 10→7.5, cls: 1.5→0.5（标准 loss 权重）
    #   6. cls_pw: 0.5→0.0（默认关闭类别权重）
    #   7. 去除两阶段训练，单阶段统一训练
    #   8. 使用完整 UAVDT 数据集，epochs=200
    #   9. 新增 YOLO26s / RT-DETR-l 对比实验
    # ---------------------------------------------------------
    # Table 1: 核心对比 (Exp01-05)
    # Table 2: PVRP 子模块消融 (Exp06-09)
    # Table 3: Lite 副创新消融 (Exp10-11)
    # Table 4: SNAA 内部项消融 (Exp12-13)
    # Table 5: YOLO26 / RT-DETR 强基线对比 (Exp14-16)
    # =========================================================
    experiments = [
        # =====================================================
        # Table 1: 核心对比实验
